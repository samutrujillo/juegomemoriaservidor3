...
class SMSNotificationService {
    ...
    formatPlayerOnlineMessage(playerName, mesaType = "VIP") {
        ...
        return `🎮 MJAPPGAME
👤 ${playerName}
🎯 Mesa ${mesaType} 15.000
⏰ ${timeStr}
🟢 CONECTADO`;
    }

    // Formatear mensaje de jugador desconectado
    formatPlayerOfflineMessage(playerName, mesaType = "VIP") {
        const timeStr = new Date().toLocaleString('es-ES', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
...
_¡Excelente juego!_`;
    ...
    async sendSMS(message, notificationKey = null) {
        // Modo de prueba cuando se alcanza el límite diario
    ...
    async notifyPlayerOnline(playerName, mesaType = "VIP") {
    ...
    async notifyPlayerOffline(playerName, mesaType = "VIP") {
        if (!playerName || playerName === 'admin') return { success: false, reason: 'admin' };
...
if (serviceAccount) {
    admin.initializeApp({
        ...
        databaseURL: process.env.FIREBASE_DATABASE_URL || "https://juego-memoria-cuarto-default-rtdb.firebaseio.com"
    });
    db = admin.database();
...
io.on('connection', (socket) => {
  ...
  socket.on('joinGame', () => {
      ...
      if (existingPlayerIndex === -1) {
          ...
          if (!user.isAdmin) {
              ...
              smsNotificationService.notifyPlayerOnline(user.username, "VIP")
                  .then(result => {
                      if (result.success) {
                          if (result.simulated) {
          ...
          // NUEVO: SIEMPRE enviar notificación de conexión, sin importar el estado anterior
  ...
  socket.on('leaveGame', () => {
      ...
      if (playerIndex !== -1) {
          ...
          // NUEVO: Notificar desconexión del jugador (solo si no es admin)
          const user = getUserById(userId);
          if (user && !user.isAdmin) {
              console.log(`👤 ${user.username} salió del juego - enviando notificación de DESCONECTADO`);
              
              smsNotificationService.notifyPlayerOffline(user.username, "VIP")
                  .then(result => {
                      if (result.success) {
                          if (result.simulated) {
...
console.log('Base de datos Firebase:', 'https://juego-memoria-cuarto-default-rtdb.firebaseio.com');
